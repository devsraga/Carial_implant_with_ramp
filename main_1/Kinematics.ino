...
// Forward Kinematics function
void forwardKinematics_dev(long theta1_steps, long theta2_steps, long theta3_steps) {
    // Convert steps to angles
    float theta1 = stepsToDegrees(theta1_steps);
    float theta2 = stepsToDegrees(theta2_steps);
    float theta3 = stepsToDegrees(theta3_steps);
...
// Inverse Kinematics function
bool inverseKinematics_dev(float X, float Y, float Z, long &theta1_steps, long &theta2_steps, long &theta3_steps) {
    // Compute theta1 (base joint)
    float theta1 = atan2(Y, X) * 180.0 / PI;
...
void moveToPosition(float X, float Y, float Z, float mot_speed, float accel) {
    ...
    if (inverseKinematics_dev(X, Y, Z, theta1_steps, theta2_steps, theta3_steps)) {
        ...
        forwardKinematics_dev(theta1_steps, theta2_steps, theta3_steps);
    } else {
